...
@pytest.mark.parametrize('recipe', ['jpeg576_fusion_local_weighted_val_wavelet', 'jpeg576_wavelet_dual_b0', 'jpeg576_wavelet_dual_b1'])
def test_wavelet_jpeg_dataset_loaders_and_budget(tmp_path, recipe):
    import pandas as pd
    ...
    config = load_experiment_config(f'configs/{recipe}.yaml')
    ...
    with torch.device('meta'):
        ...
        baseline = build_model(replace(config.model, encoder_name='pvt_v2_b2', wavelet_image_size=0,
                                       noise_encoder_name=None), pretrained=False)
    cost = count_gflops(full, 576, native_size=(1024, 1024))
    base_cost = count_gflops(baseline, 576, native_size=(1024, 1024))
    assert cost > 0
    if recipe == 'jpeg576_fusion_local_weighted_val_wavelet':
        assert base_cost < cost < 100
    elif recipe == 'jpeg576_wavelet_dual_b0':
        assert cost < base_cost < 100
    # B1/B1 is deliberately retained as an over-budget research recipe.
