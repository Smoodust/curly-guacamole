...
def test_training_layout_keeps_variable_jpeg_convolutions_contiguous():
    ...
    model = configure_memory_format(build_model(ModelConfig(), pretrained=False))
...
def test_png_named_jpeg_bypasses_forensics_and_recompression_restores_it(tmp_path):
    ...
    fusion = ForensicFusion((4, 8, 16, 32), (4, 8, 16, 32), (8, 16, 32)).train()
    ...
    out = fusion(features, jpeg=[native.tensors()])
    ...
    mixed = [torch.randn(2, c, 64//s, 64//s, requires_grad=True)
             for s, c in zip((4, 8, 16, 32), (4, 8, 16, 32), strict=True)]
    fused = fusion(mixed, jpeg=[native.tensors(), result.jpeg.tensors()])
...
def test_dataset_collation_transfer_and_recompression(tmp_path):
    ...
    dataset = AIIJCDataset(workspace, pd.DataFrame(rows), False, 64, 42, mode='val')
...
def test_jpeg_model_forward_backward_and_required_inputs():
    ...
    torch.set_num_threads(1)
    model = build_model(ModelConfig(), pretrained=False).train()
    ...
    for block in model.forensic_fusion.fusion_blocks.values():
        block.channel_gate.data.fill_(0.1)
